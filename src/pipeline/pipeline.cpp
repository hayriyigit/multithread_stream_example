...
Pipeline::Pipeline()
{
    this->result_queue = std::make_shared<ThreadSafeQueue<std::shared_ptr<SharedData>>>();
};
...
void Pipeline::start()
{
    ...
    {
        throw PipelineExecutionException("At least one filter need to run pipeline");
    }
    ...
    for (size_t i = 0; i < this->get_caller_size(); ++i)
    {
        ...
        auto output_queue = (i + 1 != this->get_caller_size()) ? this->queues[i + 1] : this->result_queue;
        auto &filter = this->m_callers[i];

        std::visit([&](auto &visit_pointer)
                   {
            auto *raw_pointer = visit_pointer.get();
            this->threads.emplace_back([input_queue, output_queue, raw_pointer]()
                        {
                        while (true)
                        {
...
std::shared_ptr<ThreadSafeQueue<std::shared_ptr<SharedData>>> Pipeline::get_output_queue()
{
...
void Pipeline::add_filter(std::unique_ptr<Filter> filter)
{
    ...
    this->queues.push_back(std::make_shared<ThreadSafeQueue<std::shared_ptr<SharedData>>>());
};

void Pipeline::add_tmp_filter(std::unique_ptr<std::function<void(std::shared_ptr<SharedData>)>> tmp_filter)
{
