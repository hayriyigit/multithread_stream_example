...
TEST_F(PipelineTest, TestAddFilter_Functional)
{
    ...
    auto fn = std::make_unique<std::function<void(std::shared_ptr<SharedData>)>>([&](std::shared_ptr<SharedData>) {});
...
TEST_F(PipelineTest, TestAddFilterOrder)
{
    ...
    {
        const auto &uptr = std::get<std::unique_ptr<std::function<void(std::shared_ptr<SharedData>)>>>(m_callers[0]);
...
TEST_F(PipelineTest, TestPiepelineBehavior)
{
    ...
    auto fn1 = std::make_unique<std::function<void(std::shared_ptr<SharedData>)>>([&](std::shared_ptr<SharedData> data)
                                                                                  {
                                                                                        std::string val = std::any_cast<std::string>((*data)["debug"]);
                                                                                        (*data)["debug"] = val + "1"; });

    auto fn2 = std::make_unique<std::function<void(std::shared_ptr<SharedData>)>>([&](std::shared_ptr<SharedData> data)
                                                                                  {
                                                                                        std::string val = std::any_cast<std::string>((*data)["debug"]);
                                                                                        (*data)["debug"] = val + "2"; });
